...
//  SwiftBootcamp
//
//  Created by Назар Горевой on 18.07.2023.
//
...
struct CodableBootCamp: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            if let custommer = vm.customer {
                Text(custommer.id)
                Text(custommer.name)
                Text("\(custommer.points)")
                Text(custommer.isPremium.description)
            }
        }
    }
}
